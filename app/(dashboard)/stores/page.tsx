    ...
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-xl sm:text-2xl font-heading font-bold text-gray-900">Manajemen Cabang</h1>
            <p className="text-gray-500 text-sm mt-1">Kelola stok dan data setiap cabang toko roti Anda</p>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6">
        {stores.map((store: any) => (
          <Link href={`/stores/${store.id}`} key={store.id} className="group block">
            <div className="bg-white rounded-xl border-2 border-transparent hover:border-primary-200 shadow-sm hover:shadow-md transition-all duration-300 p-5 h-full">
              <div className="flex items-start space-x-4">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="text-base font-bold text-gray-900 group-hover:text-primary-700 transition-colors truncate">
                    {store.name}
                  </h3>
                  {store.location && (
        ...
        {stores.length === 0 && (
          <div className="col-span-full flex flex-col items-center justify-center p-10 bg-white rounded-xl border-2 border-dashed border-gray-200 text-gray-400">
            ...
            <p className="text-base font-semibold text-gray-600">Belum ada cabang toko</p>
